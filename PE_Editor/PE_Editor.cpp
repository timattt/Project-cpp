#include "PE_Editor.hpp"

// first name in file array is file name.
PE_Maker::PE_Maker(tFile *file_, int code_sz, int data_sz, int total_import_files,
...
int PE_Maker::getCodeSize() {
	return code_size;
}

int PE_Maker::getDataSize() {
	return data_size;
}

char* PE_Maker::getPointerInFile(int addr) {
	return (file + addr);
}

void PE_Maker::constructDefaultFunctions() {
...
int PE_Maker::getCodeCarriageLocation() {
	return (code_carriage - file);
}

int PE_Maker::getDataCarriageLocation() {
	return (data_carriage - file);
}

int PE_Maker::getCodeVA() {
	return IMAGE_BASE + CODE_RVA;
}

int PE_Maker::getDataVA() {
	return IMAGE_BASE + DATA_RVA;
}

int PE_Maker::getImportVA() {
	return IMAGE_BASE + IMPORT_RVA;
}

int PE_Maker::getCodeLocation() {
	return code_locat;
}

int PE_Maker::getDataLocation() {
	return data_locat;
}

void PE_Maker::flipCodeCarriage() {
	code_carriage = file + code_locat;
}

int PE_Maker::getCodeCarriageVA() {
	return -code_locat + (int) (code_carriage - file) + getCodeVA();
}

int PE_Maker::getDataCarriageVA() {
	return -data_locat + (int) (data_carriage - file) + getDataVA();
}

// writes import function into code carriage. Moves code carriage
int PE_Maker::wcif(const char *name) {
...
void PE_Maker::pushEAX() {
	wcb(0x50);
}

void PE_Maker::popEAX() {
	wcb(0x58);
}

void PE_Maker::pushRegs() {
	wcb(0x60);
}

void PE_Maker::popRegs() {
	wcb(0x61);
}

// places default out function in code section on the carriage position.
void PE_Maker::callDefaultOutFunction() {
	wcb(0xe8);
	wcdw(out_function_va - getCodeCarriageVA() - 4);
}

void PE_Maker::callDefaultInFunction() {
	wcb(0xe8);
	wcdw(in_function_va - getCodeCarriageVA() - 4);
}

void PE_Maker::callDefaultOutCharFunction() {
	wcb(0xe8);
	wcdw(out_char_function_va - getCodeCarriageVA() - 4);
}

void PE_Maker::callDefaultInStringFunction() {
	wcb(0xe8);
	wcdw(in_string_function_va - getCodeCarriageVA() - 4);
}

void PE_Maker::callImportedFunction(const char *name, char *place) {
	char *ptr = place;
	ptr[0] = 0xff;
	ptr[1] = 0x15;
...
int PE_Maker::alignFile(int val) {
	return tUp(val, fileAl) * fileAl;
}

int PE_Maker::alignRVA(int val) {
	return tUp(val, sectAl) * sectAl;
}

void PE_Maker::initSizes() {
...
void PE_Maker::initImportSection(int total_import_files,
		const char import[MAX_IMPORT_FILES][MAX_IMPORT_FUNCTIONS + 2][MAX_IMPORT_NAME_LEN]) {
...
void PE_Maker::initSectionsTable() {
	initSection(code_h,
				".text",
				CODE_SIZE_RVA,
				CODE_RVA,
...
void PE_Maker::initSection(_IMAGE_SECTION_HEADER *sec, const char *name,
				 int virt_size, int virt_addr, int size_of_raw_data,
				 int pointer_to_raw, int charact) {
	CHECK (!sec, "Section pointer is null!");
