#include "GrammarParsersCreator.hpp"

//! Writes something like this
//! EXAMPLE: smt = "abc";
//! 		(*(s + 0) == 'a' && *(s + 1) == 'b' && *(s + 2) == 'c');
tString writeEquals(tString smt, unsigned dx) {
	tString res = { "(" };

	// If there is encounter
	if (smt.tCount('-') > 0 && smt.tSize() == 3) {
		char from = smt[0];
		char to = smt[2];
		...
		for (unsigned i = 0; i < smt.tSize(); i++) {
			res += "*(s + ";
			res += tString(i + dx);
			res += ") == '";
			res += smt[i];
...
tString createChecker(tString name, tString body, tString *nonterminals,
	...
	if (line[0] == '[') {
		for (unsigned j = 0; (line.tSize() > 0 && line[0] == '['); j++) {
			...
			line = line.tSubstring(end + 1, line.tSize() - 1);
		}
	} else {
...
//! @param total_nont - tSize of this array
...
tString parseNoOr(tString name, tString body_, tString *nonterminals,
	...
	while (body.tSize() > 0) {
		...
		if (body[0] == '[') {
			...
			if (body.tCount('-') > 0
					&& body.tFirstPosition('-') < body.tFirstPosition(']')) {
				res += (tString("res += *s;\n"));
				res += (tString("\ts += 1;\n"));
			} else {
			...
			if (body.tFirstPosition(']') + 1 >= body.tSize()) {
				body = { };
			} else {
				// If there will be something
				body = body.tSubstring(body.tFirstPosition(']') + 1,
						body.tSize() - 1);
			}
		} else {
			...
			for (; j < total_nont; j++) {
				if (body.tIsPrefix(nonterminals[j])) {
					if (nonterminals[j].tSize() >= body.tSize()) {
						body = { };
					} else {
						body = body.tSubstring(nonterminals[j].tSize(),
								body.tSize() - 1);
					}
					break;
				}
			...
			if (body.tSize() > 0
				...
				if (body.tSize() == 1) {
					...
					// If not
					body = body.tSubstring(1, body.tSize() - 1);
				}
			}
...
tString removeSpacesSafe(tString from) {
	...
	for (unsigned i = 0; i < from.tSize(); i++) {
...
void splitAsNameAndBody(tString smt, tString &name, tString &body) {
	// Position before :
	unsigned eqsi = smt.tFirstPosition(':') - 1;
	// Position after ::=
	unsigned eqei = eqsi + 4;
	name = removeSpacesSafe(smt.tSubstring(0, eqsi));
	body = removeSpacesSafe(smt.tSubstring(eqei, smt.tSize() - 1));
}
